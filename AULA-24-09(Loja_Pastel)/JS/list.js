...
function buscarPastel() {
  ...
  let filtrados = pasteis.filter(p =>
    p.nome.toLowerCase().includes(termo) ||
    p.ingredientes.toLowerCase().includes(termo)
  );
  ...
  filtrados.forEach(p => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn btn-warning" onclick="edit(${p.id})">Editar</button>
        <button class="btn btn-danger" onclick="deletarPasteis(${p.id})">Excluir</button>
      </td>
    `;
    tbody.appendChild(tr);
  });
